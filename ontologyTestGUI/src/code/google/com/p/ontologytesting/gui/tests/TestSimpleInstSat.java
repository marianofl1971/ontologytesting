...
public class TestSimpleInstSat extends javax.swing.JPanel{
    ...
    private void initComponents() {
        ...
        labelInstLabel.setText(java.util.ResourceBundle.getBundle(Configuration.getPropiedades().getProperty("IDIOMA"),new Locale(Configuration.getPropiedades().getProperty("LOCALE"))).getString("CONSULTASINST_RESULTADO_ESPERADO"));
        ...
        tabbedPaneInst.addTab(java.util.ResourceBundle.getBundle(Configuration.getPropiedades().getProperty("IDIOMA"),new Locale(Configuration.getPropiedades().getProperty("LOCALE"))).getString("Formato_ayuda"), opcionAyudaInstPanel);

        opcionTextInstPanel.setLayout(new javax.swing.BoxLayout(opcionTextInstPanel, javax.swing.BoxLayout.X_AXIS));
        tabbedPaneInst.addTab(java.util.ResourceBundle.getBundle(Configuration.getPropiedades().getProperty("IDIOMA"),new Locale(Configuration.getPropiedades().getProperty("LOCALE"))).getString("Formato_Texto"), opcionTextInstPanel);
            ...
            if(tamQ==tamR){
                for(int i=0; i<tamQ;i++){
                    if(texto.consultaCompletaEnSinAyuda(cQuery[i].trim(), cResult[i].trim())==true){
                        if(continuar=true){
                            hayUnaConsulta=1;
                            if(validarTests.validarQueryInstSatis(cQuery[i].trim())==true &&
                                    validarTests.validarResultadoInstSatis(cResult[i].trim())==true){
                                ...
                                this.inst.add(i, 0);
                                validarConsultas.setListInst(this.inst);
                            }else if(validarTests.validarQueryInstSatis(cQuery[i].trim())==false &&
                                    validarTests.validarResultadoInstSatis(cResult[i].trim())==true){
                                this.inst.add(i, 1);
                                validarConsultas.setListInst(this.inst);
                                validoInst=false;
                            }else if(validarTests.validarQueryInstSatis(cQuery[i].trim())==true &&
                                    validarTests.validarResultadoInstSatis(cResult[i].trim())==false){
                                this.inst.add(i, 2);
                                validarConsultas.setListInst(this.inst);
                                validoInst=false;
                            }else{
